...
def AnswerModifier(Answer):
    ...
    modified_answer = " ".join(non_empty_lines)
...
# Function for setting current microphone status in Mic.data file if our mic is on it writes false if mic is off it writes true
def setMicrophoneStatus(Command):
    with open(rf"{TempDirPath}/Mic.data", "w", encoding="utf-8") as file:
        file.write(Command)

# Function for getting current microphone status
def getMicrophoneStatus():
    with open(rf"{TempDirPath}/Mic.data", "r", encoding="utf-8") as file:
        status = file.read()
    return status
    
# Function for setting current Assistant status
...
class ChatSection(QWidget):
    ...
    def __init__(self):
        ...
        movie = QMovie(graphicsDirectoryPath('Jarvis.gif'))            # have to add these gif in graphics
        max_gif_size_W = 480
        max_gif_size_H = 270
...
class InitialScreen(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        movie.setScaledSize(QSize(gif_label.size())) 
